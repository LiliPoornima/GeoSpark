...
const BACKGROUND_IMAGES = [solar, wind, hydro];
    ...
    try {
      ...
      if (response.status === 429 || response.status >= 500) {
        if (attempt === maxRetries - 1)
          throw new Error(
            `API failed after ${maxRetries} attempts with status: ${response.status}`
          );
      } else {
...
const ChatBubble: React.FC<{ message: Message }> = ({ message }) => {
  ...
  return (
    <div className={`flex w-full mb-4 ${alignment}`}>
      <div
        className={`max-w-3xl p-4 rounded-xl shadow-lg ${bgColor} ${textColor} ${
        ...
        {message.isLoading ? (
          ...
          <>
            ...
            {message.sources && message.sources.length > 0 && (
              <div
                className={`mt-3 pt-2 border-t text-xs opacity-80 ${
                  isUser
                    ? "border-green-400/50 text-green-100"
                    : "border-gray-400/50 text-green-700"
                }`}
              >
                <p
                  className={`flex items-center mb-1 font-semibold ${
                    isUser ? "text-white" : "text-green-700"
                  }`}
                >
        ...
        {BACKGROUND_IMAGES.map((url, index) => (
          <img
            ...
            onError={(e) => {
              ...
              e.currentTarget.src =
                "https://placehold.co/1200x800/1e293b/ffffff?text=Energy";
            }}
          />
        ))}
      </div>
      ...
      <div className="relative z-10 flex flex-col h-full max-w-4xl mx-auto p-4 sm:p-8">
        <div className="flex flex-col h-full w-full bg-white/95 backdrop-blur-sm text-gray-900 rounded-xl shadow-2xl overflow-hidden border border-gray-200">
          ...
          <div className="p-4 bg-green-600 border-b border-green-800 flex items-center shadow-md text-white">
            ...
            {/* Changed badge color */}
            <span className="ml-3 px-2 py-1 text-xs bg-green-900/50 rounded-full">
              Renewable Energy AI
            </span>
          </div>
          ...
          <div className="p-4 bg-white/95 border-t border-gray-200">
            ...
            <p className="text-xs text-gray-600 mt-2 flex items-center">
              ...
              Press Enter to send. Powered by Gemini with Google Search
              grounding.
            </p>
          </div>
